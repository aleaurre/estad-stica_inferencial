...
datos_pib <- datos_pib %>%
  ...
  filter(!is.na(pib_pc), region != "Aggregates") %>%
  mutate(pib_pc = pib_pc / 1000)

# Crear intervalos
breaks <- seq(0, max(datos_pib$pib_pc, na.rm = TRUE) + 10, by = 10)

# Generar tabla con marcas de clase
tabla_frec <- datos_pib %>%
  mutate(intervalo = cut(pib_pc, breaks = breaks, right = FALSE)) %>%
...
# ----------------------
# VARIABLE CONTINUA CON INTERVALOS DESIGUALES
# ----------------------
...
# Suponiendo que tenés una tabla local con población (iso3c, poblacion)
# poblacion <- read_csv(here("data", "poblacion.csv"))
# datos_enriquecidos <- datos_graficos_limpio %>% left_join(poblacion, by = "iso3c")
